...
if [[ "$arch" == "riscv64" ]]
then
sudo apt-get install -y libasound2-dev libpulse-dev libavutil-dev g++-riscv64-linux-gnu debootstrap cpio
# somehow you have to configure ffmpeg manually for riscv64
cd src/third_party/ffmpeg && ./configure --arch=riscv64 --disable-asm && make config && make -j16 || true && cd ../../../
